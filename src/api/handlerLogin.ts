...
import { SafeUser, User } from "../db/schema.js";
import { config } from "../config.js";
import { createRefreshToken } from "../db/queries/refresh_token.js";

export async function handlerLogin(req: Request, res: Response, expireInSeconds=3600){
    type parameters = {
        password: string;
        email: string;
    };
    ...
    if(!refreshToken) throw new BadRequestError("Could not generate refresh token");
