...
export async function createRefreshToken(token: string, user_id: string){
...
export async function findRefreshToken(rToken: string){
    const [result] = await db
        .select()
        .from(refresh_tokens)
...
export async function revokeToken(rToken: string){
    const [result] = await db
